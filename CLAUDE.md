...
**The classifier does not respect the contract**, and both known deviations are repaired in `classifyIntent()` rather than prayed against — QBert strips Ollama's `format` field, so the shape cannot be enforced at the API level:
- **Flattened params**: `{"intent":"x","action":"edit","detail":"…"}` instead of nesting under `params`. Anything left at the top level is recovered as params.
- **Invented intents**: `{"intent":"group_memory_update","updates":[…]}` — a name coined from a real agent's id. `resolveIntentName()` maps it back when exactly **one** installed agent matches on a normalised `_`-separated prefix; an ambiguous stem like `image` (image_gen / image_query) resolves to nothing and falls back, because silently picking the wrong agent is worse than the default. Both repairs are noted in `dispatcher.log` on the `RESULT:` line, so a classifier drifting further is visible rather than merely tolerated.
...
**Compaction.** Notes grow untidily: filler openings (`Il gruppo…`, `Si parla di…`), near-duplicates, and one-off chat chronicle. Two counter-measures. At insertion, the prompt bans chronicle formulations (`Si parla di` is itself the tell), one-off technical advice, and filler subjects — the section header already says whose group it is. Then `compactGroupMemory()` runs a dense rewrite, but **only above `GROUP_MEMORY_COMPACT_AT` (1500 chars)**, so rarely: it *is* a full rewrite, i.e. exactly the generation-loss risk the diff mechanism exists to avoid. It is fenced in — temperature 0.1, an explicit order to copy proper nouns letter by letter, and four refusals: an empty answer, a gain below `GROUP_MEMORY_COMPACT_MIN_GAIN` (15%), an altered proper noun, or a cut below a third of the lines. The previous text stays in history either way. First real run: 20 → 14 lines, 1739 → 1236 chars, with `root_camp_fratta`, `rootcamp.rootclub.it`, `vokoscreenNG` and `vitello dai piedi di cobalto/balsa` all intact.

The gain threshold and the proper-noun guard were added after the *second* real run corrupted the text: 16 → 16 lines, 1930 → 1785 chars, and `venerdì` came back as `veneriglio`. The old "any result shorter than the input" rule let it through. The lesson is that a rewrite gambles the integrity of the whole block, so it has to buy something: 7.5% is not enough to be worth it. `groupMemoryNotableTokens()` recognises the tokens that must be copied verbatim **by shape** — internal underscore or dot, `@`, a digit, or a non-initial capital (`root_camp_fratta`, `rootcamp.rootclub.it`, `@thefox71`, `ESP32`, `GL.iNet`) — and refuses any that appears in the output without existing in the input. Disappearing is fine (dropping lines is the point); appearing in an altered form is the signature of corruption. Words with only an initial capital are deliberately excluded: every line starts with one, and including them would block every compaction. A common word like `venerdì` cannot be guarded this way, so `groupMemoryNewWords()` merely **logs** every word ≥5 chars in the output that was absent from the input — legitimate rephrasings and corruptions land in the same list, and telling them apart in code isn't possible, but without that line the next `veneriglio` is only discoverable by re-reading the block by hand.
